...
import { UserAPICache, cache } from "../config/cache.js";
...
export const cacheControl: MiddlewareHandler = async (c, next) => {
    const sMaxAge = env.USER_API_S_MAXAGE;
    const staleWhileRevalidate = env.USER_API_STALE_WHILE_REVALIDATE;
...
export function cacheConfigSetter(keySliceIndex: number): MiddlewareHandler {
    return async (c, next) => {
        ...
        const duration = Number(
            c.req.header(UserAPICache.CACHE_EXPIRY_HEADER_NAME) ||
                UserAPICache.DEFAULT_CACHE_EXPIRY_SECONDS
        );
        c.set("CACHE_CONFIG", {
        ...
        if (UserAPICache.enabled) {
            c.res.headers.set(
                UserAPICache.CACHE_EXPIRY_HEADER_NAME,
                duration.toString()
            );
        }
